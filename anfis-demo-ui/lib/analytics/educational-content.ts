...
export const METRIC_EXPLANATIONS: Record<string, EducationalContent> = {
  ...
  'how_surrogate_model_works': {
    ...
    designDecisions: 'Why an MLP instead of directly running ANFIS at runtime?\nThe canonical ANFIS requires iterative least-squares parameter estimation per inference step, which is O(n²) in rule count. With 27 rules (3³ fuzzy inputs), this takes 50–200ms — too slow for a 30s window. The MLP forward pass is O(weights) and completes in <1ms.\n\nWhy 2 hidden layers with [16, 8] nodes?\n- 1 layer (e.g., [32]) failed to capture the non-linear interaction between membership and delta inputs (test R²=0.82)\n- 3 layers added overfitting with no accuracy gain on the test set\n- [16, 8] was the smallest architecture that achieved R² > 0.90 on the test set\n- The bottleneck structure (16 → 8) forces the network to learn a compressed representation before the output\n\nWhy ReLU activation?\nSigmoid and tanh were tested. ReLU achieved faster convergence and avoided the vanishing gradient problem in the second hidden layer. The output layer uses Linear (no activation) to allow full-range multiplier output before clamping.\n\nWhy neutral-centred calibration instead of min-max rescaling?\nMin-max rescaling requires a fixed empirical output range which is highly sensitive to extreme training inputs. A perfectly balanced player (⅓,⅓,⅓ memberships, zero deltas) must map to display=1.0 by semantic definition. Neutral-centred calibration enforces this: raw==mlp_neutral → display=1.0, deviations above/below scale symmetrically by factor 2.0. mlp_neutral is auto-recomputed by notebook 07 after each retrain — no code changes needed.\n\nWhy clamp output to [0.6, 1.4]?\nThe safety bounds prevent catastrophic difficulty spikes from out-of-distribution inputs. The lower bound 0.6× prevents the game from becoming trivially easy (still a meaningful challenge). The upper bound 1.4× prevents the game from becoming impossibly hard (a 40% increase in all enemy stats is already very significant).',
  },
