...
export function JsonEditorDialog({ title, value, onChange, description }: JsonEditorDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-2xl bg-background border-border h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-foreground flex items-center gap-2">
            {title}
            {error && <span className="text-xs text-red-600 dark:text-red-400 font-normal bg-red-500/10 px-2 py-0.5 rounded border border-red-500/30">{error}</span>}
          </DialogTitle>
          {description && <DialogDescription className="text-muted-foreground">{description}</DialogDescription>}
        </DialogHeader>
        
        <div className="flex-1 relative min-h-0 border border-border rounded-md bg-muted/30">
            <textarea
                value={internalValue}
                onChange={(e) => {
                ...
                className="absolute inset-0 w-full h-full p-4 bg-transparent text-foreground font-mono text-xs resize-none focus:outline-none focus:ring-1 focus:ring-primary/50 rounded-md"
                spellCheck={false}
