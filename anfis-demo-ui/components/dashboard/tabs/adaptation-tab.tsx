    ...
    <div className="m-0 p-4 sm:p-5 space-y-6 w-full">
      ...
      {tutorialMode && (
        <div className="p-3 rounded-lg border border-amber-500/30 bg-amber-500/5 flex items-start gap-2.5">
          ...
          <div>
            <p className="text-xs font-semibold text-amber-700 dark:text-amber-300 mb-0.5">What is this tab?</p>
            <p className="text-[11px] text-muted-foreground leading-relaxed">
              This shows <span className="text-foreground">the game settings the AI adjusted</span> based on the player's detected behaviour.
      ...
      {tutorialMode && (
        <div className="rounded-xl border border-border/50 bg-card/30 overflow-hidden">
          <div className="px-4 py-3 border-b border-border/40 flex items-center gap-2">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">How adaptation works -- the full chain</p>
          </div>
          <div className="px-4 py-4 space-y-4 text-[11px] text-muted-foreground leading-relaxed">
            ...
            <ol className="space-y-3 list-none">
              <AdaptStepRow n="1" title="Measure behavior" color="text-cyan-600 dark:text-cyan-400"
              ...
              <AdaptStepRow n="2" title="Classify play style" color="text-violet-600 dark:text-violet-400"
              ...
              <AdaptStepRow n="3" title="Compute a global multiplier" color="text-amber-600 dark:text-amber-400"
              ...
              <AdaptStepRow n="4" title="Scale each parameter by archetype weight" color="text-emerald-600 dark:text-emerald-400"
            ...
            <div className="pt-2 border-t border-border/40">
              <p className="text-muted-foreground">
                <span className="text-foreground font-medium">Why the 0.5 baseline in the formula?</span>{' '}
...
function ParameterCard({ title, children, color }: { title: string, children: React.ReactNode, color: 'combat' | 'collection' | 'exploration' }) {
  ...
  const bgGradient = 'bg-linear-to-b from-slate-900 to-slate-950 dark:from-[#161b2c] dark:to-[#0f111a]'; // Adaptive gradient
  const titleColor = color === 'combat' ? 'text-rose-600 dark:text-rose-400' : color === 'collection' ? 'text-amber-600 dark:text-amber-400' : 'text-sky-600 dark:text-sky-400';
  const glowColor = color === 'combat' ? 'shadow-rose-900/5 dark:shadow-rose-900/10' : color === 'collection' ? 'shadow-amber-900/5 dark:shadow-amber-900/10' : 'shadow-sky-900/5 dark:shadow-sky-900/10';
...
function ParameterRow({ name, base, final, format = "0.00", dark, onClick }: any) {
  ...
  return (
    <div
      onClick={onClick}
      className="flex items-center justify-between group py-1 cursor-pointer hover:bg-muted/50 dark:hover:bg-white/5 rounded pl-2 pr-1 transition-colors -ml-2"
    >
      <div className="space-y-1.5 point-events-none">
        <div className="text-[13px] font-medium text-foreground group-hover:text-primary transition-colors tracking-wide underline-offset-2 group-hover:underline decoration-dotted decoration-muted-foreground/30">
          {name}
        </div>
        <div className="font-mono text-[11px] text-muted-foreground flex items-center gap-2">
          <span className="opacity-60">Base:</span>
          <span className="text-foreground/70">{base.toFixed(2)}</span>
          <span className="text-muted-foreground/40">{"->"}</span>
          <span className="opacity-60">Final:</span>
          <span className="text-foreground font-bold bg-muted/50 px-1.5 rounded">{final.toFixed(2)}</span>
        </div>
      </div>
      ...
      {!isNeutral && (
        <div className={cn(
          ...
          isGain ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" : "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20"
        )}>
          {isGain ? '+' : ''}{percentChange.toFixed(1)}%
        </div>
      )}
      {isNeutral && (
        <div className="px-2.5 py-1 rounded-md text-[11px] font-bold font-mono min-w-14 text-center shadow-sm border border-border/50 bg-muted/50 text-muted-foreground">
          0.0%
        </div>
      )}
    </div>
...
function WindowComparison({
  ...
  if (!previousCategories || previousCategories.length === 0) {
    return (
      <div className="p-3 rounded-lg border border-border/50 bg-card/30 flex items-center gap-2 text-muted-foreground">
        <ArrowRight className="w-3.5 h-3.5 shrink-0" />
        <p className="text-[11px]">Run the simulation a second time to see a <span className="text-foreground">window-to-window comparison</span> showing what changed and why.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-card/40 overflow-hidden">
      <div className="px-4 py-3 border-b border-border/40 flex items-center gap-2">
        ...
        <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">vs. Previous Window</p>
        <EducationalDrawer
          contentKey="how_deltas_work"
          trigger={<span className="ml-auto text-[10px] text-primary hover:text-primary/70 cursor-pointer underline underline-offset-2 decoration-dotted">what is a delta?</span>}
        />
      </div>
      ...
      <div className="px-4 py-3 border-b border-border/30 flex items-center justify-between gap-4">
        <div>
          <p className="text-xs font-semibold text-foreground mb-0.5">Overall difficulty multiplier</p>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className={cn('text-xs font-bold',
                    cat.category === 'Combat' ? 'text-rose-600 dark:text-rose-400' :
                      cat.category === 'Collection' ? 'text-amber-600 dark:text-amber-400' : 'text-sky-600 dark:text-sky-400'
                  )}>{cat.category}</span>
                  <DeltaChip delta={delta} />
                </div>
                <p className="text-[11px] text-muted-foreground leading-snug truncate">{label}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-[10px] text-muted-foreground/60 font-mono">{prev ? (prev.softMembership * 100).toFixed(0) : '--'}% {"->"}  {(cat.softMembership * 100).toFixed(0)}%</p>
              </div>
            </div>
          );
        })}
      ...
      {tutorialMode && (
        <div className="px-4 py-4 border-t border-border/40 bg-card/30 space-y-2">
          <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-2">How these deltas cascade to parameters</p>
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            Each archetype percentage feeds into a <span className="text-foreground font-medium">category factor</span> (a multiplier close to 1.0).
            The factor is computed as: <span className="font-mono text-cyan-600 dark:text-cyan-400">factor = 1.0 + (globalMult − 1.0) x (0.5 + membership x 1.5)</span>.
          </p>
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            When that factor goes <span className="text-emerald-600 dark:text-emerald-400">above 1.0</span>: parameters tied to that archetype get harder/more intense
            (e.g., more enemies, less items timeout). When it goes <span className="text-rose-600 dark:text-rose-400">below 1.0</span>: those parameters ease off.
            Some parameters scale directly (<span className="font-mono text-foreground/80">base x factor</span>) and some inversely
            (<span className="font-mono text-foreground/80">base / factor</span>) to balance challenge against player power.
          </p>
...
function DeltaChip({ delta }: { delta: number }) {
  ...
  if (abs < 0.01) return <span className="text-[10px] font-mono text-muted-foreground bg-muted/60 px-1.5 rounded border border-border/50">+/-0%</span>;
  const pct = Math.round(abs * 100);
  if (delta > 0) return (
    <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-1.5 rounded border border-emerald-500/20 flex items-center gap-0.5">
      <ArrowUp className="w-2.5 h-2.5" />+{pct}%
    </span>
  );
  return (
    <span className="text-[10px] font-mono text-rose-600 dark:text-rose-400 bg-rose-500/10 px-1.5 rounded border border-rose-500/20 flex items-center gap-0.5">
...
function MultiplierDeltaBadge({ prev, curr }: { prev: number; curr: number }) {
  ...
  const abs = Math.abs(delta);
  if (abs < 0.001) return (
    <span className="text-xs font-mono text-muted-foreground bg-muted/60 border border-border/50 px-2 py-1 rounded-md whitespace-nowrap">= {curr.toFixed(3)}x</span>
  );
  if (delta > 0) return (
    <span className="text-xs font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-1 rounded-md whitespace-nowrap flex items-center gap-1">
      <ArrowUp className="w-3 h-3" />{curr.toFixed(3)}x
    </span>
  );
  return (
    <span className="text-xs font-mono text-rose-600 dark:text-rose-400 bg-rose-500/10 border border-rose-500/20 px-2 py-1 rounded-md whitespace-nowrap flex items-center gap-1">
