          ...
          <div className="p-0">
            {rules.length === 0 ? (
              ...
              rules.map((rule, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-3 p-3 border-b border-border/40 hover:bg-muted/40 transition-colors group"
                >
                  <div className="w-8 shrink-0 flex flex-col items-center gap-0.5">
                    <HelpfulTooltip
                      trigger={
                        <span className="text-[10px] font-mono text-muted-foreground/60 cursor-help hover:text-muted-foreground transition-colors">
                          STR
                        </span>
                      }
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-foreground truncate font-mono text-wrap leading-tight">
                      {rule.ruleName}
                    </p>
                    <Progress
                      value={rule.strength * 100}
                      className="h-1 w-full mt-1.5 bg-muted"
                      indicatorClassName={
                        rule.strength > 0.5
                          ? "bg-emerald-500/70"
                          : "bg-blue-500/70"
