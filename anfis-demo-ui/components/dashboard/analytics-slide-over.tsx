...
const ARCHETYPE_COLOR: Record<string, string> = { Combat: 'text-rose-600 dark:text-rose-400', Collection: 'text-amber-600 dark:text-amber-400', Exploration: 'text-sky-600 dark:text-sky-400' };
...
function deltaBadge(delta: number) {
  ...
  if (abs < 0.01) return <span className="text-[10px] font-mono text-slate-500 dark:text-slate-600 px-1">=</span>;
  if (delta > 0) return <span className="inline-flex items-center gap-0.5 text-[10px] font-mono text-emerald-600 dark:text-emerald-400"><ArrowUp className="w-2.5 h-2.5" />+{pct}%</span>;
  return <span className="inline-flex items-center gap-0.5 text-[10px] font-mono text-rose-600 dark:text-rose-400"><ArrowDown className="w-2.5 h-2.5" />-{pct}%</span>;
}
...
function PlainEnglishSummary({
  ...
  const multColor = mult > 1.02 ? 'text-rose-600 dark:text-rose-400' : mult < 0.98 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400';
  ...
  return (
    <div className="mx-2 sm:mx-4 mt-4 mb-1 rounded-xl border border-border bg-card/60 overflow-hidden">
      {/* ── Main summary row ── */}
      <div className="p-4">
        <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground mb-2">What just happened?</p>
        <p className="text-sm text-foreground leading-relaxed">
          ...
          <span className="text-muted-foreground">{ARCHETYPE_DESC[dominant.category]}</span>.
          {' '}Difficulty set to{' '}
          <span className={`font-bold ${multColor}`}>
            {multDir === 'unchanged' ? 'stay the same' : `${multDir} by ${multPct}%`}
          </span>
          {' '}
          <span className="font-mono text-muted-foreground">({mult.toFixed(3)}x</span>
        ...
        {sessionChangeText && (
          <div className="mt-2 flex items-start gap-1.5">
            <ArrowRight className="w-3 h-3 text-muted-foreground/60 shrink-0 mt-0.5" />
            <p className="text-[11px] text-muted-foreground leading-relaxed">{sessionChangeText}</p>
          </div>
        )}
        {!hasPrev && (
          <p className="text-[11px] text-muted-foreground/60 mt-2 italic">Run a second simulation to see how this window compares to the previous one.</p>
        )}
      </div>
        ...
        {categories.map(cat => (
          <div key={cat.category} className="w-full sm:flex-1 flex flex-col gap-0.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-muted-foreground">{Math.round(cat.softMembership * 100)}%</span>
            </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-2.5 border-t border-border/50 hover:bg-muted/40 transition-colors group"
      >
      ...
      {tutorialMode && expanded && (
        <div className="px-4 py-4 border-t border-border/40 bg-muted/30 space-y-4 text-[11px] text-muted-foreground leading-relaxed">

          {/* Step 1: Activity scoring */}
          <CalcSection step="1" title="Activity scoring" color="text-cyan-600 dark:text-cyan-400">
            Raw telemetry stats (kills, damage, distance, etc.) are grouped per archetype and averaged to produce
            an activity score between 0 and 1 per category. Each stat is first <span className="text-foreground/80">normalised</span> using
            training-set min/max so they are on the same scale.
          </CalcSection>

          {/* Step 2: Soft clustering */}
          <CalcSection step="2" title="Soft membership via K-Means" color="text-violet-600 dark:text-violet-400">
            ...
            centroid is computed. <span className="text-foreground/80">Inverse-distance weighting</span> converts those distances
          ...
          {hasPrev ? (
            <CalcSection step="3" title="Behavioural deltas" color="text-amber-600 dark:text-amber-400">
              Delta = current membership − previous membership. These deltas are fed into the surrogate model
              alongside current memberships so the AI can detect <span className="text-foreground/80">trends</span>, not just current state.
          ...
          <CalcSection step="4" title="Surrogate model (MLP) output" color="text-emerald-600 dark:text-emerald-400">
          ...
          <CalcSection step="5" title="Parameter cascade" color="text-rose-600 dark:text-rose-400">
            Each archetype group's parameters are scaled by a category factor:
            <span className="block font-mono text-cyan-600 dark:text-cyan-400 mt-1 text-[10px]">
            ...
            Parameters scale directly (<span className="font-mono text-foreground/80">base x factor</span>) or inversely
            (<span className="font-mono text-foreground/80">base / factor</span>) depending on whether they should increase or decrease with difficulty.
          </CalcSection>
          ...
          <div className="pt-1 border-t border-border/40">
            <div className="flex items-center gap-2">
              <EducationalDrawer
                ...
                trigger={<span className="text-[10px] text-primary hover:text-primary/70 cursor-pointer underline underline-offset-2 decoration-dotted">Surrogate model deep dive</span>}
              />
              <span className="text-muted-foreground/40">·</span>
              <EducationalDrawer
                contentKey="how_deltas_work"
                trigger={<span className="text-[10px] text-primary hover:text-primary/70 cursor-pointer underline underline-offset-2 decoration-dotted">How deltas work</span>}
              />
              <span className="text-muted-foreground/40">·</span>
              <EducationalDrawer
                contentKey="how_normalization_works"
                trigger={<span className="text-[10px] text-primary hover:text-primary/70 cursor-pointer underline underline-offset-2 decoration-dotted">Normalization</span>}
              />
            </div>
          </div>
        </div>
...
export function AnalyticsSlideOver({ open, onOpenChange }: AnalyticsSlideOverProps) {
  ...
  if (pipelineState.behaviorCategories.length === 0) {
    return (
      <Dialog.Root open={open} onOpenChange={onOpenChange}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 z-40" />
          <Dialog.Content className="fixed right-0 top-0 h-screen w-[600px] bg-background border-l border-border shadow-2xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right z-50 overflow-hidden flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-border">
              <div className="flex items-center gap-3">
                <BarChart3 className="w-5 h-5 text-primary" />
                <Dialog.Title className="text-lg font-semibold text-foreground">Analytics</Dialog.Title>
              </div>
            ...
            <div className="flex-1 flex items-center justify-center p-6">
              <div className="text-center">
                <TrendingUp className="w-12 h-12 text-muted-foreground/50 mb-4 mx-auto" />
                <p className="text-sm text-muted-foreground">Run simulation to see analysis results</p>
              </div>
            </div>
          </Dialog.Content>
        </Dialog.Portal>
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/40 dark:bg-black/60 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 z-40" />
        <Dialog.Content className="fixed inset-y-0 right-0 h-full w-full sm:max-w-2xl lg:max-w-3xl xl:max-w-4xl 2xl:max-w-3xl bg-background border-l border-border shadow-2xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right z-50 overflow-hidden flex flex-col">
          {/* Header */}
          <div className="flex items-center justify-between p-4 sm:p-6 border-b border-border/60 bg-card/50 shrink-0">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-lg bg-primary/10 border border-primary/20">
                <BarChart3 className="w-5 h-5 text-primary" />
              </div>
              <div>
                <Dialog.Title className="text-base sm:text-lg font-semibold text-foreground tracking-tight flex items-center gap-2">
                  System Analytics
                  <EducationalDrawer
                    contentKey="anfis_pipeline_overview"
                    trigger={<Info className="w-4 h-4 text-muted-foreground hover:text-primary cursor-pointer transition-colors" />}
                  />
                </Dialog.Title>
                <p className="text-xs text-muted-foreground font-mono">Real-time behavioral telemetry</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button
              ...
              <Dialog.Close className="rounded-lg p-2 hover:bg-muted transition-colors border border-transparent hover:border-border">
                <X className="w-5 h-5 text-muted-foreground" />
              </Dialog.Close>
            </div>
          </div>
