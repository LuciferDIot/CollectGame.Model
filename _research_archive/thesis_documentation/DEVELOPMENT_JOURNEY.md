...
| **Phase 9**: Training Bias & Calibration Fix | Refinement | v2.2.1: Corrected target asymmetry & added neutral-centred calibration (R^2=0.9350) |
...
| v2.2 | 0.9 | 0.22/0.18/0.15 | 0.55/0.4/0.35 | -0.25 | 0.062 | 0.41 | 0.9391 | Success |
| **v2.2.1 FINAL** | **1.0** | **0.22/0.18/0.15** | **0.55/0.4/0.35** | **-0.25** | **0.0760** | **0.5213** | **0.9350** | **Active Success** |

*Note: Soft coefficients apply to centered features (x - 0.5) in v2.2 and v2.2.1 versions.*

---

**Document Version**: 1.1
**Date**: 2026-03-07
**Status**: Thesis Documentation - Finalized with Calibration Fix
...
## Phase 9: Training Bias & Calibration Fix (v2.2.1) - March 2026

### Problem Discovery

Post-deployment testing of v2.2 showed that regardless of archetype or playstyle intensity, difficulty calculations consistently skewed to "easier" (difficulty multiplier $M < 1.0$). 

### Root Cause Analysis

1. **Structural Training Offset**:
   The Option B formula used `base = 0.9`. Because soft memberships satisfy the partition-of-unity constraint ($\sum \mu = 1.0$), centering them at $0.5$ (subtracting $0.5$ from each) introduced a fixed structural offset of $(0.22 + 0.18 + 0.15) \times (1/3 - 0.5) \approx -0.092$ for a balanced player. This pulled the target multiplier for a balanced player down to **0.808**, biasing the entire training set.
2. **Min-Max Calibration Sensitivity**:
   The initial min-max output calibration was highly sensitive to extreme input ranges generated by hypothetical deltas of $\pm 1.0$. This pulled the empirical minimum to 0.49, rendering typical, realistic player inputs permanently skewed to "harder" once shifted.

### Solution: Neutral-Centred Calibration & Bias Correction

1. **Bias Fix (`base = 1.0`)**:
   Notebook `06_ANFIS_Preparation.ipynb` was updated to set the base multiplier to `1.0`. A balanced player now has a target multiplier of $1.0 - 0.092 = 0.908$, restoring a symmetric target distribution.
2. **Neutral-Centred Calibration**:
   Instead of min-max rescaling, the display multiplier is anchored to the raw output of a balanced player (`mlp_neutral = 0.931601`), scaled by a factor of 2.0:
   $$ M_{\text{display}} = \text{clamp}(1.0 + (\text{raw} - \text{mlp\_neutral}) \times 2.0,\ 0.6,\ 1.4) $$
   This ensures that any balanced, steady-state player mathematically maps to exactly $1.0$.

### Outcome

Notebooks 06 and 07 were re-executed. The final model achieves **Test $R^2 = 0.9350$**, **Test $\text{MAE} = 0.0123$**, and **$\Delta\text{explore}$ correlation $r = -0.758$**, resolving target bias and achieving production-ready calibration.
