...
Initial formula: `M = 1.0 − 0.1xdeaths + 0.05xactivity`
...
Parameters swept: K (2,3,4,5) x outlier handling (none, IQR, Z-score) x normalization (3 variants) x feature sets (3 subsets).
...
**Result after adopting Option B**: σ increased from 0.0113 to 0.0625 (+453%). R^2 went from −4.69 (worse than mean prediction) to **0.9391** (near-perfect). Problem considered solved.
...
A second structural bug: activity scores were computed as **sums**, giving Combat (4 features) a 4x ceiling over any 1-feature category. This created a structural bias toward Combat in the normalized activity space.
...
- 1 layer [32]: Test R^2=0.82 - insufficient to capture membership x delta interactions
- 2 layers [16,8]: Test R^2=0.93 - optimal
- 2 layers [32,16]: R^2=0.91 but 4x parameters with no meaningful gain
...
|-----------|-------|------------|------------|
| R^2 = −4.69 (model worse than mean) | v1.0 | Variance collapse (σ=0.011) | Option B target formula (σ->0.062) |
| Attacker misclassified as Explorer | v2.0 | `timeOutOfCombat` passive accumulation | v2.1: removed from Exploration score |
| Sum asymmetry - Combat ceiling 4x higher | v2.0 | Raw sums, not per-archetype averages | v2.1: switched to per-archetype mean |
...
| Session reset mid-game | v2.0 | 40s timeout too short for loading screens | v2.2: increased to 90s (3x window cadence) |
